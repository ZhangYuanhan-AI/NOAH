...
from model.module.multihead_super_prompt import AttentionSuper
...
class Vision_TransformerSuper(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 super_prompt_tuning_dim=1024,super_LoRA_dim=1024,super_adapter_dim=1024):
        ...
        for i in range(depth):
            self.blocks.append(TransformerEncoderLayer(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio,
                                                       ...
                                                       max_relative_position=max_relative_position,visual_prompt_dim=super_prompt_tuning_dim,LoRA_dim=super_LoRA_dim,adapter_dim=super_adapter_dim))
        ...
        trunc_normal_(self.visual_prompt_token_pos_embed, std=.02)


        # For pixel tuning

        # self.pixel_prompt_token_w_l = nn.Parameter(torch.zeros(1, 3,self.input_resolution-padding_size,self.padding_size))
        # self.pixel_prompt_token_w_r = nn.Parameter(torch.zeros(1, 3,self.input_resolution-padding_size,self.padding_size))
        # self.pixel_prompt_token_h_t = nn.Parameter(torch.zeros(1, 3,padding_size,self.input_resolution-self.padding_size))
        # self.pixel_prompt_token_h_d = nn.Parameter(torch.zeros(1, 3,padding_size,self.input_resolution-self.padding_size))
    ...
    def _freeze_stages(self):
        ...
        print('parameters:',total_para_nums,'adapter',adapter_para_nums,'LoRA',LoRA_para_nums,'prompt',vp_para_nums)
        ...
        for i, blocks in enumerate(self.blocks):
            ...
            if i < self.sample_layer_num:
                ...
                blocks.set_sample_config(is_identity_layer=False,
                                        ...
                                        sample_last_prompt_tuning_dim = self.sample_prompt_tuning_dim[i-1] if i>0 else self.sample_last_prompt_tuning_dim[0],
    ...
    def forward_features(self, x):
        ...
        #     visual_prompt_tokens = 0*self.visual_prompt_token + 0*self.visual_prompt_token_pos_embed
        #     x += visual_prompt_tokens
        # else:
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, dropout=0., attn_drop=0.,
                 ...
                 visual_prompt_dim=1024,LoRA_dim=1024,adapter_dim=1024):
        super().__init__()
        ...
        self.attn = AttentionSuper(
            ...
            max_relative_position=max_relative_position, LoRA_dim=LoRA_dim
        )
    ...
    def forward(self, x):
        ...
        if self.sample_visual_prompt_dim == 0:
            # import pdb;pdb.set_trace()
            # visual_prompt_tokens = 0*self.visual_prompt_token
            # x += visual_prompt_tokens
            residual = x
        else:
            # 
