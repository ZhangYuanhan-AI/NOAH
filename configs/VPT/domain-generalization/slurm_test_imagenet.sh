...
PARTITION='dsta'
JOB_NAME=test
CONFIG=experiments/VPT/ViT-B_prompt_vpt_50.yaml
GPUS=1
INPUT_DIR=$1
WEIGHT_DECAY=0.0001
...
do 
    ...
    do
        ...
        srun -p ${PARTITION} \
            ...
            2>&1 | tee -a logs/${currenttime}-${DATASET}-${LR}-vpt-test.log > /dev/null & 
            echo -e "\033[32m[ Please check log: \"logs/${currenttime}-${DATASET}-${LR}-vpt-test.log\" for details. ]\033[0m"
    done
done
