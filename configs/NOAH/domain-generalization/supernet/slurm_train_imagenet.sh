        ...
        do
            ...
            do 
                ...
                srun -p ${PARTITION} \
                    ...
                    2>&1 | tee -a logs/${currenttime}-${DATASET}-${LR}-${SEED}-${SHOT}-dg-supernet.log > /dev/null & 
                    echo -e "\033[32m[ Please check log: \"logs/${currenttime}-${DATASET}-${LR}-${SEED}-${SHOT}-dg-supernet.log\" for details. ]\033[0m"
            done
        done
    done
done