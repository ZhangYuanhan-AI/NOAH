...
for DATASET in stanford_cars fgvc_aircraft oxford_pets food-101  oxford_flowers 
do 
    ...
    do 
        ...
        srun -p ${PARTITION} \
            ...
            2>&1 | tee -a logs/${currenttime}-${DATASET}-${SHOT}-fs-search.log > /dev/null & 
            echo -e "\033[32m[ Please check log: \"logs/${currenttime}-${DATASET}-${SHOT}-fs-search.log\" for details. ]\033[0m"
    done
done
