...
for LR in 0.005 
do 
    for DATASET in fgvc_aircraft oxford_pets food-101 stanford_cars oxford_flowers 
    do 
        ...
        do
            ...
            do
                ...
                srun -p ${PARTITION} \
                    ...
                    2>&1 | tee -a logs/${currenttime}-${DATASET}-lr-${LR}-shot-${SHOT}-seed-${SEED}-fs-rt.log > /dev/null & 
                    echo -e "\033[32m[ Please check log: \"logs/${currenttime}-${DATASET}-lr-${LR}-shot-${SHOT}-seed-${SEED}-fs-rt.log\" for details. ]\033[0m"
            done
        done
    done
done