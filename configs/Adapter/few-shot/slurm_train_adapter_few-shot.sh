        ...
        do
            ...
            do 
                ...
                srun -p ${PARTITION} \
                    ...
                    2>&1 | tee -a logs/${currenttime}-${DATASET}-${LR}-seed-${SEED}-shot-${SHOT}-adapter.log > /dev/null & 
                    echo -e "\033[32m[ Please check log: \"logs/${currenttime}-${DATASET}-${LR}-seed-${SEED}-${SHOT}-shot-adapter.log\" for details. ]\033[0m"
          done
        done
    done
done
